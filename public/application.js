...
var onClickResetSession = function onClickResetSession() {
  sessionGamesX = 0;
  sessionGamesO = 0;
  sessionGamesDraw = 0;
  updateSession();
...
var addWinValue = function addWinValue(winValue) {
  $('.board-grid').addClass(winValue + '-won');
  $('.player-turn-line').html('player ' + winValue + ' wins!').addClass('' + winValue);
};

var gameNotice = function gameNotice(over, winValue) {
...
module.exports = {
  ...
  onClickResetSession: onClickResetSession
};
...
var playerOAiTurn = function playerOAiTurn(over) {
  ...
  if (store.winningX === false) {
    // Check for match during human play…
...
var animateGameBoard = function animateGameBoard() {
...
var setNewGame = function setNewGame() {
  ...
  if (store.user !== null) {
    api.createGame().then(function (result) {
      // console.log(result)
      gameData = result;
    }).catch(function (err) {
      console.log(err);
...
var onRetrieveOverGames = function onRetrieveOverGames() {
  event.preventDefault();
  var statsOverGames = null;
  var xStatsWins = null;
  var oStatsWins = null;
  var drawGames = null;
  api.retrieveOverGames().then(function (result) {
    statsOverGames = result;
    // console.log(statsOverGames)
    // console.log(over)
    for (var i = 0; i < statsOverGames.games.length; i++) {
    ...
    var totalGamesPlayed = xStatsWins + oStatsWins + drawGames;
...
var newGamePrep = function newGamePrep(event) {
  $('.game-information-area').show();
  $('.intro-text').hide();
  onClickNewGame(event);
};

var toggleNavDrawer = function toggleNavDrawer() {
  $('.nav-drawer').slideToggle(200);
};

var addGameHandlers = function addGameHandlers() {
  $('.game-title').click(function (event) {
    $('.intro-text').toggle();
  });
  $('.play-new-game').click(function (event) {
    // If humanGame remains false then the new game will be against the computer.
    newGamePrep(event);
  });
  $('#new-game-human').click(function (event) {
    // This handler sets the condition for a 2-person, human-to-human game.
    humanGame = true;
  });
...
module.exports = {
  ...
  animateGameBoard: animateGameBoard,
...
var checkForMatch = function checkForMatch(cells, over, turnCounter, gameData, cellsIndex, playerPiece, onClickCell) {
  var winValue = null;
  ...
  for (var _i = 0; _i <= 2; _i = _i + 1) {
    if (cells[_i] !== '' && cells[_i] === cells[_i + 3] && cells[_i] === cells[_i + 6]) {
      winValue = cells[_i]; // columns win
      over = true;
    }
  }
  ...
  console.log(cells);
  if (store.user !== null) {
    api.updateGame(gameData, cellsIndex, playerPiece, over).then(function (result) {
      // console.log(result)
    }).catch(function (err) {
      console.log(err);
    });
...
var checkStatsForMatch = function checkStatsForMatch(cells, statsWinValue) {
  for (var i = 0; i <= 6; i = i + 3) {
    if (cells[i] !== '' && cells[i] === cells[i + 1] && cells[i] === cells[i + 2]) {
      statsWinValue = cells[i].toUpperCase() + ' won'; // rows win
    }
  }
  for (var _i2 = 0; _i2 <= 2; _i2 = _i2 + 1) {
    if (cells[_i2] !== '' && cells[_i2] === cells[_i2 + 3] && cells[_i2] === cells[_i2 + 6]) {
      statsWinValue = cells[_i2].toUpperCase() + ' won'; // columns win
    }
  }
  if (cells[4] !== '' && (cells[0] === cells[4] && cells[0] === cells[8] || cells[2] === cells[4] && cells[2] === cells[6])) {
...
module.exports = {
  ...
  checkStatsForMatch: checkStatsForMatch
};
/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(0)))
...
var updateGame = function updateGame(gameData, index, value, over) {
...
var retrieveOverGames = function retrieveOverGames() {
  return $.ajax({
...
module.exports = {
  ...
  retrieveOverGames: retrieveOverGames
};
/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(0)))
...
var ellipticalWait = function ellipticalWait() {
...
var signOutSuccess = function signOutSuccess(animateGameBoard, onClickResetSession, onClickCell) {
  store.preGame = true;
  boardEvents.animateGameBoard();
  gameEvents.onClickResetSession();
...
var randomCellRun = function randomCellRun(cells) {
...
var cellCheck = function cellCheck(cells, onClickCell) {
...
var makeAiCellPlayed = function makeAiCellPlayed(cells, onClickCell) {
...
var aiTurn = function aiTurn(cells, over, turnCounter, gameData, cellsIndex, playerPiece, onClickCell) {
